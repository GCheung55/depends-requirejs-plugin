...
(function(){
	...
	function getRequirements(list, useOrder){
		var order = (useOrder==true)?"order!":"";
		var reqs = [];
		if(list){
			for(var i=0; i<list.length; i++){
				if(is_string(list[i])){
					reqs.push(order+list[i]);
	...
	function attachAdditionalPlugins(plugins){
		if(is_array(plugins)){
	...
	define({
	    ...
	    load: function (name, req, load, config) {
			if(config.mootools){
				...
				attachAdditionalPlugins(config.mootools.plugins);
			}
