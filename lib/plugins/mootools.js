...
(function(){
	...
	function attachCoreModules(path){
		var coreModules = [
			{
				name: "Core",
				path: path,
		...
		for(var i=0; i<coreModules.length; i++){
			attachModule(coreModules[i]);
		}
	}
	//MooTools More Package
	function attachMoreModules(path){
		var moreModules = [
			{
				name: "More",
				path: path,
		...
		for(var i=0; i<moreModules.length; i++){
			attachModule(moreModules[i]);
		}
	}
	
	function attachAdditionalModules(modules){
		if(is_array(modules)){
			for(var i=0; i<modules.length; i++){
				attachModule(modules[i]);
			}
		}
	}
	...
	define({
	    ...
	    load: function (name, req, load, config) {
			if(config.mootools){
				...
				attachCoreModules(corePathVariable);
				attachMoreModules(morePathVariable);
				attachAdditionalModules(config.mootools.modules);
			}
