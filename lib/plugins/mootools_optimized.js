...
//
(function(){
	var corePathVariable = "mcore";
	var morePathVariable = "mmore";
	...
	function attachCorePlugins(path){
		var corePlugins = [
			{
				name: "Core",
				path: path,
				files: [
					...
					{ name: "Element/Element", provides: ["Element", "Elements", "$", "$$", "Iframe", "Selectors"], requires: ["Browser", "Array", "String", "Function", "Object", "Number", "Slick.Finder"] },
					...
					{ name: "Element/Element.Dimensions", provides: "Element.Dimensions", requires: "Element.Style" },
					{ name: "Element/Element.Delegation", provides: "Element.Delegation", requires: "Element.Event" },
					{ name: "Fx/Fx", provides: "Fx", requires: "Class.Extras" },
					...
					{ name: "Utilities/DOMReady", provides: ["DOMReady", "DomReady"], requires: ["Browser", "Element.Event"] },
					{ name: "Utilities/Swiff", provides: "Swiff", requires: ["Options", "Object", "Element"] },
					{ name: "Request/Request", provides: "Request", requires: ["Object", "Element", "Class.Extras", "Browser"] },
		...
		for(var i=0; i<corePlugins.length; i++){
			attachPlugin(corePlugins[i]);
		}
	}
	//MooTools More Package
	function attachMorePlugins(path){
		var morePlugins = [
			{
				name: "More",
				path: path,
				files: [
					...
					{ name: "Class/Array.Extras", provides: "More/Array.Extras", requires: ["Array", "More"] },
					{ name: "Class/Object.Extras", provides: "More/Object.Extras", requires: ["Object", "More"] },
		...
		for(var i=0; i<morePlugins.length; i++){
			attachPlugin(morePlugins[i]);
		}
		console.log( source );
	}
	...
	define({
	    ...
	    load: function (name, req, load, config) {
			if(config.mootools){
				corePathVariable = config.mootools.corePathVariable || corePathVariable;
				morePathVariable = config.mootools.morePathVariable || morePathVariable;
				attachCorePlugins(corePathVariable);
				attachMorePlugins(morePathVariable);
				if(is_array(config.mootools.plugins)){
					var plugins = config.mootools.plugins;
					for(var i=0; i<plugins.length; i++){
						attachPlugin(plugins[i]);
					}
				}
			}
